...
1. **Deploy Sprint 1 to VPS.** Prompt.py + engine.py + chronicler.py changes are on master but not live. Run PRE-DEPLOY CHECKLIST in CLAUDE.md end to end. No new env vars. Prompt-and-engine-only change — no database migration, no new routes. Smoke test with a TEST- key after deploy (flow only — TEST mode does not exercise the new prompt content; first real session will). [2026-04-21]
