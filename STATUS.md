...
Last updated: 2026-04-26 (spine regeneration verification + YAML truncation issue surfaced)
...
60. **YAML truncation on unquoted values — issue tracked, fix scheduled** — Surfaced 2026-04-26 during the NlF6dc4mdobt regeneration recorded in entry 59. PyYAML validation failed at line 155 with "while parsing a block mapping — expected block end, but found scalar". Trigger: the YAML extraction model emitted an unquoted string value containing nested quotes and a colon, e.g. `user_says: "time with my wife" is non-negotiable value` — PyYAML reads `"time with my wife"` as the start of a quoted key, then sees `is non-negotiable value` and throws because that's a scalar where a key was expected. Recovery: the existing line-by-line recovery path in `generate_yaml_sections()` truncated to 154 valid lines; the tail of the spine (parts of `relationships`, `long_game`, and possibly later sections) was lost. File size impact: 14,322 chars vs ~20,454 chars expected from a clean regeneration. Pre-existing — not caused by the 2026-04-25 stitcher fix; the truncation pattern would have surfaced against any session producing this kind of value. The stitcher fix is what made it visible by pushing real-session regeneration to top of mind. Severity contained: live sessions are unaffected because the defensive recovery prevents catastrophic failure (truncated > corrupted). Fix path: tighten output formatting instructions in `YAML_EXTRACTOR_PROMPT` to require full quoting of any value containing nested quotes, colons, or special characters. Scheduled for the next sprint, not an emergency hotfix. Tracked in KNOWN_ISSUES.md §3 "Known model behaviour issues". [2026-04-26]

59. **Spine regeneration verification — stitcher fix confirmed working in production** — Real-session regeneration of NlF6dc4mdobt (86 exchanges) confirms the 2026-04-25 stitcher fix (commit `31df371`) holds in production. Verification command per the original brief: `python3 -c "import yaml; data = yaml.safe_load(open('/home/scout/spines/NlF6dc4mdobt_2026-04-25.yaml')); print(type(data['spine']['meta'])); print(data['spine']['meta'])"` returned `<class 'dict'>` and `{'session_date': '2026-04-25', 'anonymous_id': 'anon_20260425_001', 'scout_version': '2.0'}`. `spine.meta` is now a properly nested dict — the pre-fix bug (children parsed as siblings of `spine:`) is closed in production. Two new meta fields surfaced — `anonymous_id` and `scout_version` — added to the SCHEMA_CONTRACTS.md §meta evidence base; the section already flags `[VERIFY on next production session — may vary by session]`, this regeneration adds two more confirmed fields to that note. 86 exchanges confirms the fix held under realistic input volume, not just the synthetic demo case. [2026-04-26]

58. **P0 stitcher indentation fix — section children now nest correctly under spine** — `_stitch_yaml_sections()` in `scout/engine.py` only. The pre-fix logic added two leading spaces to root-level keys (no leading whitespace) but left already-indented child lines untouched. Result: child lines (which arrived from the model at column 2) ended up at the same depth as the section key after the +2 shift, making them YAML siblings of `spine:` rather than children of their section. PyYAML parsed `spine.meta` as `None` and surfaced `session_date`, `schema_version` etc. as flat keys under `spine:`. This caused the production spine for key NlF6dc4mdobt (2026-04-25) to load with all section bodies missing — the bug that blocked SCHEMA_CONTRACTS.md verification yesterday. Fix: shift every non-empty line by two spaces, preserving relative indentation. Empty lines pass through unchanged. Verified deterministically via test_output/stitcher_demo.py — old logic returns `spine.meta` as `NoneType`; new logic returns it as `dict` containing `session_date` and `schema_version`. Real-session regeneration of NlF6dc4mdobt blocked locally (transcript exfiltration would violate SOUL.md custody) — will happen on VPS via admin dashboard Generate & Deliver after Pope deploys this commit. Verification command per brief: `python3 -c "import yaml; data = yaml.safe_load(open('/home/scout/spines/NlF6dc4mdobt_2026-04-25.yaml')); print(type(data['spine']['meta'])); print(data['spine']['meta'])"` — expected post-regen: `<class 'dict'>` and the meta object. [2026-04-25]

57. **DELETE_TRANSCRIPTS_ON_BURN flag with deliverable gate — DEC-SCOUT-017 amended** — Replaced the hardcoded transcript retention in `/burn` with a config-controlled flag plus a deliverable verification gate. (a) app.py: new `_deliverables_complete(key) -> bool` helper that globs `spines/{key}_*.yaml`, `spines/{key}_*_portrait_delivery.pdf`, `spines/{key}_*_meridian_delivery.pdf` and returns True only when all three are present. `/burn` now reads `DELETE_TRANSCRIPTS_ON_BURN` via `os.getenv("DELETE_TRANSCRIPTS_ON_BURN", "false").lower() == "true"` — fail-safe default is retention if the env var is absent or unset. When the flag is true AND `_deliverables_complete(key)` returns True, `delete_transcript(key)` fires. When the flag is true but deliverables are incomplete, deletion is skipped and a warning logged: "Transcript retained for {key} — deliverables incomplete, deletion skipped." When the flag is false, `delete_transcript` never runs — current beta behaviour. (b) scout/database.py: `cleanup_session()` remains a permanent no-op as before. Deletion logic lives in `/burn`, not in cleanup. (c) CLAUDE.md: DELETE_TRANSCRIPTS_ON_BURN added to the .env var list with note "false during beta and development, true at commercial launch — DEC-SCOUT-017". (d) DECISIONS.md: DEC-SCOUT-017 amended with Modified 2026-04-24 tag. Decision rewritten to reflect config-controlled + deliverable-gated behaviour; reasoning updated to explain why the config flag + gate is safer than the hardcoded disable. Deploy-time action: Pope adds `DELETE_TRANSCRIPTS_ON_BURN=false` to VPS .env before deploy. If the env var is omitted, default is retention — the fail-safe path. [2026-04-24]
