...
- **Scout system prompt** (`scout/prompt.py`) — ~1,455 lines across 8 sections: Identity & Disposition, Hard Rules (10 rules), How Scout Listens, Pre-Layer Arrival (5 register-dependent opening variations, 3-exchange max, runway not flight), Seven Layers (Roles, Work, People, Body, Beliefs, Shadows, Long Game), The Closing (closing acknowledgement, final questions, settling conversation), Parsing Pass (full spine.yaml schema, 7 parsing rules), Safety (11 hard constraints). [MODIFIED 2026-04-09: added arrival pre-layer, updated line count and section count]
